...
El SSOT del encadenamiento es este archivo + `MECANICA-CIRCUITO.md`.

## Meta-PLAN (PRO) vs BUILD (FLASH)

Todo workflow de /circuito sigue esta estructura:
...
1. El Meta-PLAN ejecuta SIEMPRE en PRO, sin importar el modo en que se invocó /circuito
2. BUILD ejecuta SIEMPRE en FLASH, incluso si /circuito se invocó en PRO
...
- `completo`: Ciclo completo con agentes/skills — Meta-PLAN → BUILD (agentes + updoc + salud + version + doctor)
...
3. **SUGERIR /doctor**
   ...
   - Si /doctor detectó correcciones no aplicadas: preguntar "¿Ejecutar /circuito doctor para aplicar?"
...
1. **META-PLAN (PRO)**
   ...
   - Safe-path: si INDEX.md ausente o labels stale → preguntar "¿Ejecutar /circuito updoc primero?"
...
3. **SUGERIR /circuito doctor** si /doctor detectó correcciones no aplicadas
...
- /circuito es el único punto de entrada para ejecución multi-comando
- Cada comando puede ejecutarse standalone (sin /circuito) — pero no habrá handoff automático
...
- `doc/mecanicas/MECANICA-CIRCUITO.md`
